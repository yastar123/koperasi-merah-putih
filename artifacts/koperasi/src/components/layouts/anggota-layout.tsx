        ...
        <Sidebar variant="inset">
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
                    const isActive = href === "/anggota/dashboard" ? location === href : location.startsWith(href);
                    return (
          ...
          <SidebarFooter className="border-t p-3">
            <div className="flex items-center gap-2 p-2 rounded-lg hover:bg-muted/60 transition-colors">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-green-50 text-green-700 text-xs font-bold shrink-0 ring-1 ring-green-100">
                {user?.nama?.slice(0, 2).toUpperCase() || "AG"}
              </div>
              <div className="flex flex-col min-w-0 flex-1">
                <span className="text-sm font-semibold truncate">{user?.nama}</span>
                <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">Anggota</span>
              </div>
